...
Luau does not have a simple `table` type, instead, table types are defined using `{}` syntax. One way of defining tables is using the `{type}` syntax which defines a list type.
