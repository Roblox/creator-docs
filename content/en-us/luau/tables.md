...
To iterate over an array, you can use [general iteration](https://luau-lang.org/performance#optimized-table-iteration) in a `for` loop. Because the arrays have numerical indices, you can also use a numeric `for` loop from **1** to the length of the array (`#array`).
