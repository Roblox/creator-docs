...
- [Opt in](#opt-in-and-enroll-items) through a one-time flow for existing games.
...
3. Complete the one-time opt-in flow. If managed pricing detects hard-coded prices in your game, you're prompted to fix them first.
...
- **New games and newly created items** are enrolled in Managed pricing by default. Item-level opt-out is available at any time.
...
Managed pricing uses [price optimization](./price-optimization.md) to identify better prices for passes and developer products based on signals from your own game, including demand, conversion, spend, and your game's spender penetration in specific countries. Items below the experiment-eligibility threshold for price optimization tests are unaffected.
...
For details on how economic location is determined, displaying regional prices in your game, and protecting in-game trades and gifts with `Class.MarketplaceService.GetUsersPriceLevelsAsync|GetUsersPriceLevelsAsync`, see [regional pricing](./regional-pricing.md).
