...
:--- | :---
`Class.Workspace` | Represents the game's 3D world. This location works well for server scripts that attach directly to objects and control their behavior.
...
`Class.StarterPlayer` ⟩ `Class.StarterPlayerScripts` | Contains general-purpose `Class.LocalScript\|LocalScripts` that run when the player joins the game.
