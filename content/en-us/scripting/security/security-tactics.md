...
- `Class.DataStore|DataStores` can only accept valid UTF-8 characters. Sanitize all strings provided by the client to ensure they are valid using `utf8.len`. Invalid UTF-8 strings can also be present in tables as keys and values. `utf8.len` will return the length of a string treating unicode characters as a single character, if an invalid UTF-8 character is encountered it will return `nil` and the position of the invalid character.
