...
When publishing eyelashes and eyebrows, you must parent these `Class.Accessory` objects under the avatar character's `Class.Model` prior to uploading the character `Model` to the Marketplace. These `Accessory` face objects must adhere to Roblox's accessory requirements and include layered components such as caging, rigging, and skinning components. For an overview on creating your own eyelashes and eyebrows, see [Create face accessories](../../art/characters/facial-animation/create-face-accessories.md). Eyebrows and eyelashes must use [Automatic Skinning Transfer](../../avatar/automatic-skinning-transfer.md) with the `RBX_Leader` and `RBX_Follower` special joints to be published on the marketplace.
...
2. Use the [Accessory Fitting Tool](../../avatar/accessory-fitting-tool.md) and select the **Eyelash** or **Eyebrow** model and click **Next**.
